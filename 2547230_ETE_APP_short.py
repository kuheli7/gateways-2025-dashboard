...
def show_key_insights(df: pd.DataFrame) -> None:
    ...
    card = lambda t: f'<div style="background-color:#E0F2F1;padding:15px;border-radius:5px;margin-bottom:15px;border-left:5px solid #1C818D;"><span style="color:#004D40;font-size:16px;">{t}</span></div>'
    c1, c2 = st.columns(2)
...
def show_feedback_samples(df: pd.DataFrame) -> None:
    ...
    st.dataframe(view[["Student Name", "College", "Event Name", "Rating", "Feedback on Fest", "Feedback Sentiment"][:15]], use_container_width=True, hide_index=True)
